...
int main(int argc,char** argv){
    ...
    vector<unsigned int> pri(6,0), pyr(5,0);
    ...
    SurfPrism prism(pri);
    vector<vector<unsigned int> > sub_eles = prism.getSubElements(pri_in_nodes);
    
    cout << "for the prism we got:\n";
    
    for (unsigned int i=0; i<sub_eles.size(); i++) {
        vector<unsigned int> elem = sub_eles[i];
        cout << "new element";
