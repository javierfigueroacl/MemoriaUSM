...
namespace Clobscode
{
	BoundaryTemplate5::BoundaryTemplate5(){
	}
	
	BoundaryTemplate5::~BoundaryTemplate5(){
	}
	
	bool BoundaryTemplate5::getSubelements(vector<unsigned int> &all, 
		...
		if(total==10){
			...
			PatternA(rotated,pts,newpts,neweles);
			return true;
		}
		else if(total==8){
	...
	vector<unsigned int> BoundaryTemplate5::getEdges(int p){
	...
	void BoundaryTemplate5::PatternA(vector<unsigned int> &all, vector<MeshPoint> &pts,
									 list<MeshPoint> &newpts,
	...
	vector<unsigned int> BoundaryTemplate5::searchPivote(vector<unsigned int> &doubles){
